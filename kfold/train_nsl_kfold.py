...
for train_index, val_index in kf.split(train):
    k=k+1
    model_desc='AE{}-{}_{}_{}-fold{}'.format(args.size,args.dec_rate,args.l_dim,args.num_layers,k)
    #Split
    #Training Model
    ...
    #Preprocess
    train_t_df,y_train_t,y_train_t_types,scaler,num_desc=preprocess(train_t,serviceData,flagData)  
    train_v_df,y_train_v,y_train_v_types,_,_=preprocess(train_v,serviceData,flagData,is_train=False,scaler=scaler, num_desc=num_desc)
    ...
    print("Filter Train: Normal:{}, Atk:{}".format(x_train_t[y_train_t==0].shape[0],x_train_t[y_train_t==1].shape[0]))
    ...
    for epoch in range(args.epoch):
        epoch_loss = []
        print(f"\nTraining Epoch {epoch+1} Fold {k} Seed {args.seed}")
        model.train()
...
if args.dist_cos:
    joblib.dump(best_scaler, "./weights_{}_{}_{}/nsl_{}_{}_{}_{}_{}_cos.scaler".format(args.num_layers,model_dec_type,args.dec_rate,size_val,args.l_dim,args.epoch,args.batch_size,args.seed)) 
    pickle.dump(best_num_desc,open("./weights_{}_{}_{}/nsl_{}_{}_{}_{}_{}_cos.numdesc".format(args.num_layers,model_dec_type,args.dec_rate,size_val,args.l_dim,args.epoch,args.batch_size,args.seed), 'wb'))
    # best_num_desc.to_csv(,header=None,index=None)
    with open("./weights_{}_{}_{}/nsl_{}_{}_{}_{}_{}_cos.pt".format(args.num_layers,model_dec_type,args.dec_rate,size_val,args.l_dim,args.epoch,args.batch_size,args.seed), "wb") as f:
        torch.save(
            {
...
if args.dist_cos:
    best_num_desc=pickle.load(open("./weights_{}_{}_{}/nsl_{}_{}_{}_{}_{}_cos.numdesc".format(args.num_layers,model_dec_type,args.dec_rate,size_val,args.l_dim,args.epoch,args.batch_size,args.seed), 'rb'))
    best_scaler=joblib.load("./weights_{}_{}_{}/nsl_{}_{}_{}_{}_{}_cos.scaler".format(args.num_layers,model_dec_type,args.dec_rate,size_val,args.l_dim,args.epoch,args.batch_size,args.seed))
    with open("./weights_{}_{}_{}/nsl_{}_{}_{}_{}_{}_cos.th".format(args.num_layers,model_dec_type,args.dec_rate,size_val,args.l_dim,args.epoch,args.batch_size,args.seed),'w') as f:
...
#Testing Code Moved to separate File
# if args.dec:
#     layers=[]
#     for i in range(0,args.num_layers):
#         layers.append(int(x_train.shape[1]*args.dec_rate**(i+1)))
...
# if args.dist_cos:
#     with open("./weights_{}_{}_{}/nsl_{}_{}_{}_{}_{}_cos.pt".format(args.num_layers,model_dec_type,args.dec_rate,size_val,args.l_dim,args.epoch,args.batch_size,args.seed), "rb") as f:
#         best_model = torch.load(f)
#     model.load_state_dict(best_model["state_cos"])
# else:
...
# if args.dist_cos:
#     test_dist=distance.cosine(x_test[i],pred_test[i]) for i in range(x_test.shape[0])]
# else:
