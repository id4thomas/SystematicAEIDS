...
#Train Data
df=pd.read_csv(data_dir+'/KDDTrain+.txt',header=None)
train=df.sample(frac=0.9)
val=df.drop(train.index)

print("Train",train.shape)
train.to_csv(save_dir+'/train.csv',header=None,index=False)

print("Val",val.shape)
val.to_csv(save_dir+'/val.csv',header=None,index=False)

df=pd.read_csv(data_dir+'/KDDTest+.txt',header=None)
df.to_csv(save_dir+'/test.csv',header=None,index=False)